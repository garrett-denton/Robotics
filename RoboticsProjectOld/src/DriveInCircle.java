...
public class DriveInCircle implements Behavior {
	...
	public void action() {
		try {
			...
			while (MyEv3Robot4.leftMotor.isMoving() && !suppressed)
				Thread.yield();
		} catch (Exception e) {
	...
	public void circle() {
		try {
			...
			MyEv3Robot4.leftMotor.setSpeed(leftSpeed);
			MyEv3Robot4.rightMotor.setSpeed(rightSpeed);
			// Tells motors to drive forward
			MyEv3Robot4.leftMotor.forward();
			MyEv3Robot4.rightMotor.forward();
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
	}
